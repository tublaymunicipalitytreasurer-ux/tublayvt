window.exportToExcel = function exportToExcel() {
    ...
    // Determine in-memory violations array (top-level let doesn't create window property)
    ...
    rows.forEach(row => {
        ...
        // Fallback: try to read from cells (handles cases where in-memory data isn't available)
        const cells = row.querySelectorAll('td');
        if (cells.length === 0) return;

        // Try to map available cells to headers. Collapsed rows have fewer cells.
        ...
        let offense = '';
        if (cells[4]) {
            // If expanded row, cells[4] is section; if collapsed, cells[4] is section too.
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div style="background: #fff; padding: 32px 24px; border-radius: 8px; box-shadow: 0 2px 16px #0002; min-width: 320px;">
    ...
    document.getElementById('excelExportOk').onclick = () => {
        ...
        // Fallback: ensure long text in columns E/F contains explicit newlines
        ...
        // Insert explicit newlines into long text for columns E (4) and F (5)
        ...
        // Ensure columns E (index 4) and F (index 5) have wrapText for all rows
        ...
        // Set column widths, make E and F wider to better display wrapped content
