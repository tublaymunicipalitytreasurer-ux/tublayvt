...
let tableBody, searchInput, searchBtn, addNewBtn, exportBtn, manageOffensesBtn;
...
function renderTable(data) {
    ...
    if (typeof data === 'undefined' || data === null) data = window.currentRenderedData || violations;
...
function sortTable(column) {
    ...
    renderTable(window.currentRenderedData || violations);
}
...
async function handleSubmit(e) {
    ...
    // Date is optional; allow empty/null
    ...
    const offensesSelect = document.getElementById('offenses');
    ...
    const fine = parseInt(document.getElementById('fine').value) || 0;
    const status = document.getElementById('status').value || 'Unpaid';


    if (!no || !name || !plateNumber || !sectionUuid || !offenseUuid || !level || !fine) {
    ...
    const exactDuplicate = violations.find(v => 
        v.plateNumber && v.plateNumber.toUpperCase().trim() === normalizedPlateNumber &&
        v.name === name &&
        v.offenses === offenseText &&
        v.section_id === sectionUuid && // Ensure section is the same
        v.date === date &&
        v.level === level &&
        (appState.currentEditId ? v.id !== appState.currentEditId : true)
    );
    ...
    if (exactDuplicate) {
        if (appState.currentEditId) {
            ...
            alert(`A duplicate violation already exists with the same name, plate number, offense, date, level, and section. Please check existing records.`);
        }
        return;
    }

        if (appState.currentEditId) {
            const duplicateNo = violations.find(v => 
                v.no === no && 
                v.id !== appState.currentEditId
            );
            if (duplicateNo) {
            ...
            if (duplicateNo) {
                alert(`Number ${no} is already assigned to another violation. Please use a different number.`);
                return;
            }
        }
        ...
        const violation = {
            ...
            offenses: offenseText,
            offense_id: offenseUuid,
            level,
            fine,
            status
        };
        ...
        if (appState.currentEditId && violation.status === 'Paid') {
        ...
        let result;
        if (appState.currentEditId) {
            result = await updateViolation(appState.currentEditId, violation);
        } else {
            violation.status = 'Unpaid';
            result = await addViolation(violation);
        }
        ...
        if (result.success) {
            ...
            showToast(
                appState.currentEditId ? 'Violation updated successfully!' : 'Violation added successfully!',
                'success'
            );
...
async function handlePaymentSubmit(e) {
    ...
    try {
        console.log('Marking violation as paid:', {
        ...
        if (result.success) {
            ...
            showToast(` Marked as paid: ${escapeHTML(violation.name)} - ${escapeHTML(violation.offenses)}`, 'success');
            appState.currentViolationId = null;
        } else {
